...
type TransitionMode = 'cut' | 'fade' | 'fade_white';
...
type Scene = {id:number; narration:string; visual_prompt:string; duration:number; status:string; beat?:string; continuity?:string; transition?:TransitionMode; transition_duration?:number; audio_mode?:AudioMode; generation_mode?:GenerationMode; clip?:string; source_ids?:number[]};
...
export default function Home() {
  ...
  const [transitionEdit, setTransitionEdit] = useState<Record<number, TransitionMode>>({});
  const [transitionDurationEdit, setTransitionDurationEdit] = useState<Record<number, number>>({});
  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    const poll = async () => {
  ...
  async function updateTransition(scene:Scene) {
    if (!id || !project) return;
    const mode = transitionEdit[scene.id] ?? scene.transition ?? 'cut';
    const duration = mode==='cut' ? 0 : (transitionDurationEdit[scene.id] ?? scene.transition_duration ?? 0.8);
    setError('');
    try {
      ...
      if (!res.ok) throw new Error(await res.text());
      setProject(await res.json());
      setTransitionEdit(prev=>{const next={...prev};delete next[scene.id];return next;});
      setTransitionDurationEdit(prev=>{const next={...prev};delete next[scene.id];return next;});
    } catch(e) { setError(String(e)); }
  }
  ...
  return <main>
    ...
    <section className="grid"><div className="panel"><h2>Create a project</h2>
      ...
      <p className="note">For real content select Ollama, LTX 2.5 and Kokoro. Fast uses the Distilled pipeline; Quality uses DFR and is slower with higher VRAM demand. Preview mode makes simple colored test clips with silent audio.</p>
      ...
      {project && <><div className="status"><strong>{project.status.toUpperCase()}</strong><span>{project.stage}</span></div><div className="id">Project {project.id}</div>
        ...
        <div className="scenes">{project.scenes.map((s,index)=><article key={s.id}><div className="scene-heading"><strong>Scene {s.id}{s.beat?` · ${s.beat}`:''}</strong><small>{s.duration.toFixed(1)}s · {s.status}{project.request.video_provider==='ltx25'?` · ${s.generation_mode??project.request.generation_mode??'fast'}`:''}</small></div><p>{s.narration}</p>{!!s.source_ids?.length && <div className="links">Sources: {s.source_ids.map(sourceId=>{const source=project.research?.find(item=>item.id===sourceId);return source && <a key={sourceId} href={source.url} target="_blank" rel="noopener noreferrer">{source.title} ↗</a>;})}</div>}<label>Audio<select value={audioModeEdit[s.id]??s.audio_mode??'narration'} onChange={e=>setAudioModeEdit({...audioModeEdit,[s.id]:e.target.value as AudioMode})}><option value="narration">Narration</option><option value="hybrid" disabled={project.request.video_provider!=='ltx25'}>Hybrid · narration + native ambience</option><option value="native" disabled={project.request.video_provider!=='ltx25'}>Native · LTX audio</option></select></label>{index>0 && <div className="story"><h3>Transition into this scene</h3><div className="row"><label>Transition<select value={transitionEdit[s.id]??s.transition??'cut'} onChange={e=>setTransitionEdit({...transitionEdit,[s.id]:e.target.value as TransitionMode})}><option value="cut">Cut</option><option value="fade">Fade through black</option><option value="fade_white">Fade through white</option></select></label><label>Duration (s)<input type="number" min="0.2" max="2" step="0.1" disabled={(transitionEdit[s.id]??s.transition??'cut')==='cut'} value={transitionDurationEdit[s.id]??s.transition_duration??0.8} onChange={e=>setTransitionDurationEdit({...transitionDurationEdit,[s.id]:Number(e.target.value)})} /></label></div>{project.status==='complete' && <button className="secondary" onClick={()=>updateTransition(s)}>Apply transition</button>}</div>}{s.continuity && <p className="note"><strong>Continuity:</strong> {s.continuity}</p>}<textarea aria-label={`Scene ${s.id} visual prompt`} value={edit[s.id]??s.visual_prompt} onChange={e=>setEdit({...edit,[s.id]:e.target.value})} rows={3}/>{s.clip && <a href={asset(s.clip)}>View clip ↗</a>}{project.status==='complete' && <div className="links"><button className="secondary" disabled={index===0} onClick={()=>moveScene(s.id,-1)}>Move earlier</button><button className="secondary" disabled={index===project.scenes.length-1} onClick={()=>moveScene(s.id,1)}>Move later</button><button className="secondary" onClick={()=>redo(s)}>Regenerate scene</button></div>}</article>)}</div>
      </>}
