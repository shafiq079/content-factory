    ...
    <section className="grid"><div className="panel"><h2>Create a project</h2>
      ...
      {project && <><div className="status"><strong>{project.status.toUpperCase()}</strong><span>{project.stage}</span></div><div className="id">Project {project.id}</div>
        ...
        {!!project.research?.length && <div className="story"><h3>Research evidence</h3>
          <p className="note">Source passages are context for the script, not automatic fact checking. Review claims and licensing before publishing.</p>
          {project.research_brief?.limitations.map((note,index)=><p className="note" key={index}>{note}</p>)}
          {project.research_brief?.conflicts.map((item,index)=><p className="error" key={index}>Review possible disagreement: {item.summary}</p>)}
